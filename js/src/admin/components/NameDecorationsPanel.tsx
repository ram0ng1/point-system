...
export default class NameDecorationsPanel extends Component {
  ...
  view() {
    ...
    return (
      <div className="PointSystemAdmin-section">
        ...
        {this.items.length === 0 && (
          <p className="PointSystemAdmin-empty">{app.translator.trans('ramon-point-system.admin.name.none')}</p>
        )}
        ...
        <div className="PointSystemAdmin-decoGrid">
          {this.items.map((d) => this.renderItem(d))}
        </div>
      </div>
    );
  }
    ...
    return (
      <div className={`PointSystemAdmin-decoCard ${enabled ? '' : 'is-disabled'}`}>
        <div className="PointSystemAdmin-decoCard-preview">
          <span className={`ps-name-preview ps-name-${String(slug).replace(/[^a-zA-Z0-9_-]/g, '')}`}>
            {name || 'Username'}
          </span>
        </div>
        <div className="PointSystemAdmin-decoCard-meta">
          <strong>{name}</strong>
          <small>
            {app.translator.trans('ramon-point-system.admin.name.preset')}: {preset || '—'} ·{' '}
            {price} pts
          </small>
        </div>
  ...
  renderEditForm(deco: any, draft: any) {
    const slug = deco.attribute('slug');
    const previewStyle = draft.preset === 'custom' && !String(draft.customCss).includes('{')
      ? this.parseInlineCss(draft.customCss)
      : null;
    ...
    return (
      <div className="PointSystemAdmin-decoCard PointSystemAdmin-decoCard--editing">
        <div className="PointSystemAdmin-decoCard-preview">
          <span
            className={`ps-name-preview ps-name-${String(slug).replace(/[^a-zA-Z0-9_-]/g, '')}`}
            style={previewStyle}
          >
            {draft.name || 'Username'}
          </span>
  ...
  parseInlineCss(css: string): Record<string, string> {
    ...
    css.split(';').forEach((decl) => {
      ...
      let val = decl.slice(idx + 1).trim().replace(/[<>]/g, '');
      if (!prop) return;
